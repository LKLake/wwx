...
void VelPlusOne(double **u,double **X,double **upie,double **v,double **vpie,double **inisc,
				double **inisvel,double **inifc,int N,int M,int NN,int MM,int N_sec,int M_sec,
				double delta_x,double delta_y,double ss,double delta_t,int ni,int nj)
{
	...
	for(i=0;i<ni;i++)
	{
		...
		for(j=N_sec;j<N_sec+NN+1;j++)
		{
			for(k=M_sec;k<M_sec+MM+1;k++)
			{
				...
				B[i][0]=B[i][0]+delta_x*delta_y*D[i][l]*upie[j][k];
				B[i][1]=B[i][1]+delta_x*delta_y*D[i][l]*vpie[j][k];
			}
		}
	...
	for(j=N_sec;j<N_sec+NN+1;j++)
	{
		for(k=M_sec;k<M_sec+MM+1;k++)
		{
			...
			for(i=0;i<ni;i++)
			{
				upie[j][k]=upie[j][k]+delta_t/rho*X[i][0]*D[i][l]*ss;
				vpie[j][k]=vpie[j][k]+delta_t/rho*X[i][1]*D[i][l]*ss;
			}
		}
	}
    
    /*将速度upie[i][j]，vpie[i][j]赋值给相应的整个流体区域节点，即为下一时间步速度u[i][j]，v[i][j]*/
    for(i=0;i<N+1;i++)
	{
		for(j=0;j<M+1;j++)
		{
